...
class Solution:
    def getHint(self, secret: 'str', guess: 'str') -> 'str':
        ...
        for i in range(len(secret)):
            if secret[i] in sD: sD[secret[i]].add(i)
            else: sD[secret[i]] = set([i])
            if guess[i] in gD: gD[guess[i]].add(i)
            else: gD[guess[i]] = set([i])
        bull, cow = 0, 0
        for g in gD:
            if g in sD:
                common = gD[g] & sD[g]
                bull += len(common)
                cow += min(len(gD[g]-common), len(sD[g]-common))
